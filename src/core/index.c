...
gboolean index_toggle(Index *index) {
...
// gint        fd
// NodeIndex*  index
static gint process_index (Index* index, canmsg_t* rxmsg)
{
...
Index* new_index(guint id,guint index,guint subindex){
    Index* index = g_new0(Index,1);
     index->w.flags    = 0;
     index->w.cob      = 0;
     index->w.id       = (id & 0x7F) + 0x600;
     index->w.length   = 8;
     index->w.data [1] = (char)(index & 0xff);
     index->w.data [2] = (char)(index >> 8);
     index->w.data [3] = (char)(subindex);
}
	enum {
			_LONG  = 1,
			_ANY   = 2,
...
Index*    string_index               ( guint id, guint index, guint subindex, const gchar *value){
    Index *index = new_index(id,index,subindex);
    index.value = strlen(value);
    cm . data [5] = (char)(index.value >> 8);
    cm . data [6] = (char)(index.value >> 16);
    cm . data [7] = (char)(index.value >> 24);
}
...
void read_index() {
    ...
    guint fd = open (path, O_RDWR | O_NONBLOCK);
    if (index->fd > 0) {
        write (fd, &cm, 1);
        ...
        while(TRUE){
			canmsg_t rxmsg;
            if (read (fd, &rxmsg, 1) > 0) {
                canmsg_t tx;
                memset (&tx, 0, sizeof(canmsg_t));
                tx.id = rxmsg.id
                // 0000 0000 : 0x40
                // 0000 0000 : 0x40
                // 0000 0000 : 0x40
                ...
                switch (rxmsg.data[0] & 0xE0) {
                    case 0x0:
                    case 0x10:
                     //FIXME:   node_index_init_rdata_package(index, rdata, wdata)) return FALSE;
                        cm.flags=0;cm.cob= 0;cm.length= 8;
                        break;
                    case 0x20:
                     //FIXME:    if (!node_index_init_wdata_package(index, wdata)) return FALSE;
                        cm.flags  = 0; cm.cob    = 0; cm.length = 8;break;
                    case 0x40:
                        if (rx.data[0] & _ANY) {
                        switch (rx.data[0] & 12) {
                            case _1B:g_value_set_uint (index->value, rxmsg -> data [4]); break;
                            case _2B:g_value_set_uint (index->value,  rxmsg->data[4] + (rxmsg->data[5] << 8)); break;
                            case _3B:g_value_set_uint (data->index, rxmsg->data[4] + (rxmsg->data[5] << 8) + (rxmsg->data[6] << 16)); break;
                            case _4B:g_value_set_uint (data->index, rxmsg->data[4] + (rxmsg->data[5] << 8) + (rxmsg->data[6] << 16) + (rxmsg->data[7] << 24)); break;
                            case
                            default:
                                break;
                        }
                    ...
                    case 0x41:
					//FIXME: if (node_index_object_toggle (NODE_INDEX_OBJECT(data->index), &rxmsg->data[0], &cm.data[0])) {
						cm.flags  = 0; cm.cob    = 0; cm.length = 8;
                    case 0x60:
                    }
                    case 0x70:
                        if ( rx.data[0] == 0x9 || (rx.data[0] & _LONG&& !rx.data[0]&_ANY))
					//FIXME: (node_index_object_toggle (NODE_INDEX_OBJECT (data->index), &rxmsg->data[0], &cm.data[0]))
                        cm.flags  = 0; cm.cob    = 0; cm.length = 8; break;
				}
                //FIXME: write only when long message
                write (index->fd, &cm, 1);
            }
            gdouble sec = g_timer_elapsed (timer, &microseconds);
            if (sec > 1.0 || microseconds > 200000)
				break;
            g_usleep (2);
        }
...
guint write_string( ) {
	...
	if (write (device->priv->fd, &cm, 1) > 0) {
		GTimer* timer = g_timer_new();
		g_timer_start (timer);
		while (! done) {
			gulong   microseconds = 0;
			gdouble  sec          = g_timer_elapsed(timer, &microseconds);
			canmsg_t rxmsg;
			if ((read (device->priv->fd, &rxmsg, 1)) > 0) {
				IterateData data;
				gint        ret;
				...
				ret = node_device_process_index (&data, &rxmsg);
				...
				switch (ret) {
					...
					case PROCESS_INDEX_NODE_RESETED:
						broadcast_node_reset (device, rxmsg.id & 0x07f);
						done = TRUE;
						break;
				}
			}
			else if (sec > 1.0 || microseconds > 100000)
				break;
	...
	/* Old format:
	 * case 0x40 : cm.data[0] = 0x40;   msg->val  = 0;         break;  //'g'
	   case 0x41 : cm.data[0] = 0x41;   msg->val  = 0;         break;  //'g'
	   case 0x2F : cm.data[0] = 0x2F;   msg->val &= 0xff;      break;  //'b'
	   case 0x2B : cm.data[0] = 0x2B;   msg->val &= 0xffff;    break;  //'w'
	   case 0x27 : cm.data[0] = 0x27;   msg->val &= 0xffffff;  break;  //'t'
	   case 0x23 : cm.data[0] = 0x23;   msg->val &= 0xffffffff;break;  //'q'
	   case 0x21 : cm.data[0] = 0x21;   msg->val &= 0xffff;    break;  //'s'
	   default:    cm.data[0] = 0;      msg->val  = 0;         break;
	 */
    /*
    		case 0x1:
		case 0x2: cm . data [0] = 0x2F; break;  // 1 Byte
		case 0x3: cm . data [0] = 0x2B; break;  // 2 Byte
		case 0x4: cm . data [0] = 0x23; break;  // 4 Byte
		case 0x5: cm . data [0] = 0x2F; break;  // 1 Byte
		case 0x6: cm . data [0] = 0x2B; break;  // 2 Byte
		case 0x7: cm . data [0] = 0x23; break;  // 4 Byte
		case 0x8: cm . data [0] = 0x23; break;  // 4 Byte
		case 0x9: cm . data [0] = 0x21; ret = write_string(index,error); break;  // n Byte
		default:  cm . data [0] = 0x23; break;
        */
