...
unsigned char	rtr;
int		length;
char		*format; /* format specifier for timestamp and frame format */
    ...
    s          = &databuf[0];
    got        = 0;
    if( select(FD_SETSIZE, &rfds, NULL, NULL, ( timeout > 0 ? &tv : NULL ))
	    > 0 && FD_ISSET(fd,&rfds) ) {
	...
	if( got > 0) {
	  for(i = 0; i < got; i++) {
		...
		length = rx[i].length;
		format = "%12lu.%06lu 0x%08lx :%c%c[%2d]: ";
	    } else {
		length = rx[i].length & 0x0f;
		format = "%12lu.%06lu 0x%08lx :%c%c(%2d): ";
	    }
	    ...
	    s += sprintf(s, format,
		    rx[i].timestamp.tv_sec,
		    rx[i].timestamp.tv_usec,
		    rx[i].id, type, rtr, length );      
	    if(rtr == 'D') {
		if( !(rx[i].flags & MSG_CANFD)) {
		    /* restrict to classic 8 bytes */
