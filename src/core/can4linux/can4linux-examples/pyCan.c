...
int can_open(int port, char block);


/* open the CAN device in non-blocking mode */
int can_open(int port, char block)
{
char dev[100];
...
int      j, i = 0, sent=0;
    ...
    memcpy(newMsg,message,strlen(message));

    if(( token = strtok(newMsg, ":") ) != NULL ) {
        tx.flags = 0;
	...
	if( got > 0) {
	  for(i = 0; i < got; i++) {
	    ...
	    /*s += sprintf(s, "%12lu.%06lu 0x%08lx :%c%c(%d): ",
            ...
            /* Now it works like the acutal horch */      
	    s += sprintf(s, "%12lu.%06lu  0x%08lx : %c%c : ",
		    rx[i].timestamp.tv_sec,
		    rx[i].timestamp.tv_usec,
		    rx[i].id, type, rtr );      
	    if(rtr == 'D') {
		for(j = 0; j < rx[i].length; j++) {
		    /*s += sprintf(s, " 0x%02x", rx[i].data[j]);*/
                    /* the normal horch send hex without 0x prefix */ 
		    s += sprintf(s, "%02x ", rx[i].data[j]); 
		}
	    }
#ifdef ASCII_AT_THE_END
	    for( ; j < 8; j++) {
...
/**
* can_read - read only one message in simplified format
*
* should be used on a file descriptor with data available
*
* all values returned are decimal numbers, for CAN-Id, and data
* Format like this:
* 1824 :sD(1):  127
