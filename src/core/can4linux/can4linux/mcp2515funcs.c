 ...
 * (c) 2008-2014 Heinz-Juergen Oertel (hj.oertel@t-online.de)
 *
 ...
 * to improve speed/performance where contributed by M.Hasewinkel
 *
 * SPI Introduction
 * http://108.163.188.242/~jumpnowt/index.php?option=com_content&view=article&id=57&Itemid=62
 *
 * to Imrove the driver, look at 
 * https://github.com/msperl/mcp2515a/blob/master/mcp2515a.c
*/
...
#  define up(x)           local_irq_restore(L_flags)
#  define down(x)         local_irq_save(L_flags)
# else
...
struct device_driver mcp251x_priv_drivername = {
	.name = "can4linuxspi",
	.bus = &spi_bus_type,
};

/* ================= SPI direct hardware control ===================== */
#if !defined(MCP2515SPI)
/* If the standard SPI sub layer is not used
 * code is needed to initialize the port pins used for SPI.
 * This might be very target dependent.
 *
 * The code below was used already on two ATMEL At91x platforms.
 */
...
#error "please check your Target definition, must be SSV_MCP2515 or AuR_MCP1515"
	...
	if (NULL == request_mem_region(0xfffa4000ul, 0x4000, "SPI-CAN-IO")) {
		pr_err("Request_mem_region CAN-IO failed at %lx\n",
		0xfffa4000ul);
		return -EBUSY;
	}
...
static inline void spihw_rwn_CSon(
		    unsigned char *out,
		    unsigned char *in,
		    unsigned char len
		    )
...
#endif /* ! MCP2515SPI */
/* ================= /end SPI direct hardware control ===================== */

/* SPI write one byte, activating CS */
static inline void spihw_wr8(int minor, unsigned char out)
{
struct mcp251x_priv *priv = &realone;
int ret;

	DBGIN();
	priv->spi_tx_buf[0] = out;
...
/* int minor = 0; */
	 ...
	 * To be in _sync_ with the requests
	 */
	ret = spi_sync(spi, &m);
	if (ret < 0)
...
/* device minor not used yet.
 * For more than one device using different SPI channels
 * minor has to be used to select the mcp251x_priv struct.
 */
static void mcp251x_write_bits(int minor /* struct spi_device *spi,*/, u8 reg,
			u8 mask, uint8_t val)
{
struct mcp251x_priv *priv =  &realone;
...
/* oe: must be moved to some board specific source code */
     ...
     */
	if (IRQ[minor] == 86) {
		pr_info("doing A&R specific hardware reset\n");
	...
	spihw_wr8(minor, INSTRUCTION_RESET);
...
#ifdef DEBUG
int can_show_stat(int board)
{
	if (proc_dbgmask && (proc_dbgmask & DBG_DATA)) {
		pr_info(" %s: CTRL 0x%x,", __func__, CANin(board, CANCTRL));
		pr_info(" STAT 0x%x,",  CANin(board, CANSTAT));
		pr_info(" INTF 0x%x,",  CANin(board, CANINTF));
		pr_info(" INTE 0x%x\n", CANin(board, CANINTE));
		pr_info("mask   %0x %0x %0x %0x\n",
			CANin(board, RXM0SIDH),
			CANin(board, RXM0SIDL),
			CANin(board, RXM0EID8),
			CANin(board, RXM0EID0));

		pr_info("filter %0x %0x %0x %0x\n",
...
int loopcount;
	...
	if (loopcount <= 0) {
		pr_err(
		"loop count exceeded (#1) while waiting for can status, CANSTAT is %02x\n",
	 ...
	 */
	mcp251x_write_bits(minor, RXBCTRL(0), RXBCTRL_BUKT, RXBCTRL_BUKT);
	...
	if (loopcount <= 0) {
		pr_err(
		"loop count exceeded (#2) while waiting for can status, CANSTAT is %02x\n",
		mcp251x_read_reg(CANSTAT));
		return -1;
	}
...
int isopen;
	...
	if (isopen > 1)  {
		DBGPRINT(DBG_DATA, (
		"refused to change bit rate, driver already in use by another process"
		));
		return -1;
	}
...
int ret	   = -1;
	...
	if ((isopen > 1) && (proc_baud[minor] != baud)) {
		...
		if (custom) {
			dev_info(&priv->dev,
				"%s, no custom bit register setting yet\n",
				__func__);
		} else {
...
int loopcount = 1000;
	...
	DBGIN();
	proc_rxerr[minor] = proc_txerr[minor] = 0L;   /* empty error counters */
	/* clear interrupts */
	/* nothing to do ? */
...
when the MCP2515 is in Configuration mode
(see Section 10.0 "Modes of Operation").
...
u32 sid, eid, ext, rtr;
	...
	stx_buf[0] = INSTRUCTION_LOAD_TXB(TRANSMIT_OBJ);
	if (ext) {
		eid  = tx->id & CAN_EFF_MASK; /* Extended format ID */
		stx_buf[1] = (eid >> 21) & 0xff;
		stx_buf[2] = ((eid >> 13) & 0xe0)
			+ 0x08 + ((eid & 0x30000) >> 16);
		stx_buf[3] = (eid & 0xff00) >> 8;
		stx_buf[4] = (eid & 0xff);
	} else {
	...
	stx_buf[5] = (rtr << 6) | tx->length;

	/* copy data to spi buffer */
	memcpy(stx_buf + 6, tx->data, tx->length);
	...
	/* this is the last step to tell the controller
	   sending out the frame configured with the previous spi
...
u32 sid, eid, ext, rtr;
	...
	spihw_wr8(minor, MCP_RTS_TX0);
	up(&priv->spi_lock);
	/*
	 * Save last message that was sent.
	 * Since can4linux 3.5 multiple processes can access one CAN interface.
	 * On a CAN interrupt this message is copied into the receive queue
	 * of each process that opened this same CAN interface.
	 */
	memcpy(
		(void *)&last_tx_object[minor],
		(void *)tx,
...
int rx_fifo;		/* looping through these FIFO indices */
	...
	if (can_waitflag[minor][rx_fifo] == 1) {
		...
		if (RxFifo->head == RxFifo->tail) {
			if (*printk_limit) {
				/* restrict number of overall msgs */
...
struct spi_message m;
	 ...
	 */
	stx_buf[0] = INSTRUCTION_READ_RXB(buf_idx);
	spi_message_init(&m);
	...
	rx.length = srx_buf[5] & 0x0f;
	rx.length = rx.length > 8 ? 8 : rx.length;
#else   /* use hard register access */

	stx_buf[0] = INSTRUCTION_READ_RXB(buf_idx);
	spihw_rwn_CSon(priv->spi_tx_buf, priv->spi_rx_buf, 6);
	/* Data length */
	rx.length = srx_buf[5] & 0x0f;
	rx.length = rx.length > 8 ? 8 : rx.length;
	...
	if ((srx_buf[2] >> 3) & 0x1) {
		...
		rx.id =
			...
			+ ((srx_buf[2] & 0xe0) << 13)
			+ ((srx_buf[2] & 3) << 16)
			+ (srx_buf[3] << 8) + srx_buf[4];
	} else {
		/* Base ID format */
		rx.id = (srx_buf[1] << 3) | (srx_buf[2] >> 5);
	}
	...
	memcpy(&rx.data[0], srx_buf + 6, rx.length);

	/* put the received frame in all rx queues */
	insert_rxfifo(minor, &rx, printk_limit);
...
/* mcp251x_irq_work_handler
   ...
   The following SPI actions are performed:
	...
	mcp251x_write_bits(minor, CANINTF, irqstate, 0);
...
unsigned int rx_fifo;				/* loop index for different rx queues */
int rxc = 0;
unsigned long flags;

#if 0
static int state;        /* CAN Controller state */
int newstate = 0;
#endif
int minor = 0;	/* only one device supported (yet) */
...
int printk_limit = MAX_PRINTK_LINES;          /* limit printk lines */
...
#if 0
	dev_info(&priv->dev, "interrupt:%x : %s%s%s%s%s%s%s%s\n",
		irqsrc,
	...
	if (irqsrc & CANINTF_ERRIF) {
      ...
      /*========== receive interrupt */
	    ...
	    /* read the frame and store it in the rx fifo */
	...
	if (irqsrc & (/*CANINTF_TX2IF | CANINTF_TX1IF |*/ CANINTF_TX0IF)) {
	    ...
	    memcpy((void *)&last_tx_object[minor],
		    ...
		    sizeof(canmsg_t));
#endif
	    can_send_message(minor, &TxFifo->data[TxFifo->tail]);
...
/*
  ...
  The kernel is not able to differentiate, therefore we handle both issues.
...
unsigned long flags = 0;
	...
	if ((oldtime < mytime) || (oldrx < rx)) {
		pr_err("IRQTIME:%dus RX:%d\n", mytime, rx);
		oldtime = mytime;
		oldrx = rx;
	}
#else
	/* Schedule bottom half */
	// pr_info("=====> handle CAN IRQ / %s()\n", __func__);
...
/* This is the basic function called to check if SPI is available
   ...
   struct spi_device 
   is described in include/linux/spi/spi.h
...
int minor = -1;
	...
	/* At the moment we use ONE global defined structure for the
	   ...
	   It contains e.g. the rx/tx buffers used, a semaphore,
	...
	*/
	...
	int i = 4; 	/* bit timing table index 4 is 125 k */ 
	    ...
	    /* and using the can_send_message() function */
	    tx.id = 111;
	    tx.flags = MSG_BASE;
	    tx.length = 7;
	    tx.data[0] = 1;
...
struct spi_driver mcp251x_can_driver = {
	.driver = {
		.name		= "mcp2515" /* "can_spi" */,
		.owner		= THIS_MODULE,
	},

	.probe		= mcp251x_can_probe,
	.remove		= mcp251x_can_remove,
	/* suspend and resume are not implemented anyway */
	/*
