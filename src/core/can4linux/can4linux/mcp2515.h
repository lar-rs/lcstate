...
/* structure need to hold the SPI buffers for
 * communication with the CAN using the Linux kernel SPI layer
 *
 * FIXME: needed for each CAN controller connected to the SPI bus
 */
struct mcp251x_priv {
	...
	/* the pointer is only used, when the driver handles SPI by itself */
	void *spi_iomem;
