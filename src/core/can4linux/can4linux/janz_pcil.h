...
/* CAN controller register access macros */
#ifdef IODEBUG
#  define CANOUT(bd, adr, v)	\
	(printk("Cout: (%x)=%x\n", (u32)&((canregs_t *)proc_base[bd])->adr, v), \
		writeb(v, (u32) &((canregs_t *)proc_base[bd])->adr))

#define CAN_SET(bd, adr, m)     do	{\
	unsigned char v;	\
        v = (readb((void __iomem *)&((canregs_t *)proc_base[bd])->adr)); \
	printk("CANset %x |= %x\n", (v), (m)); \
	writeb(v | (m), (u32) &((canregs_t *)proc_base[bd])->adr); \
	} while (0)

#define CAN_RESET(bd, adr, m)	do {\
	unsigned char v; \
        v = (readb((u32) &((canregs_t *)proc_base[bd])->adr)); \
	printk("CANreset %x &= ~%x\n", (v), (m)); \
	writeb(v & ~(m), (u32)&((canregs_t *)proc_base[bd])->adr); \
	} while (0)
...
#define CAN_OUTL(bd, adr, v)	\
	(printk("Cout: (%x)=%lx\n", (u32)&((canregs_t *)proc_base[bd])->adr, v),\
		(writel(v, (u32) &((canregs_t *)proc_base[bd])->adr)))
#else
   /* Memory Byte access */
#define CAN_OUT(bd, adr, v)	\
		(writeb(v, (void __iomem *)&((canregs_t *)proc_base[bd])->adr))
#define CAN_SET(bd, adr, m)	\
	writeb((readb((void __iomem *)&((canregs_t *)proc_base[bd])->adr)) \
		| (m), (void __iomem *)&((canregs_t *)proc_base[bd])->adr)
...
#define CAN_IN(bd, adr)		\
		(readb ((void __iomem *)&((canregs_t *)proc_base[bd])->adr))
#define CAN_TEST(bd, adr, m)	\
	(readb((void __iomem *) &((canregs_t *)proc_base[bd])->adr) & (m))
