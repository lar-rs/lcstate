 ...
 *------------------------------------------------------------------
 * $Header: /z2/cvsroot/products/0530/software/can4linux/src/TouCAN.h,v 1.1 2006/04/21 16:25:56 oe Exp $
 *
 *--------------------------------------------------------------------------
 *
 * 
...
typedef struct {
    volatile unsigned short     canmcr;			/*     0  */
    volatile unsigned short     cantcr;			/*        */
    volatile unsigned short     canicr;			/*     4  */
    volatile unsigned char      canctrl0;		/*        */
    volatile unsigned char      canctrl1;		/*        */
    volatile unsigned char      presdiv;		/*     8  */
...
struct can_obj {
  ...
  union {
    unsigned short int std_id;	/* base frame format Identifier			*/
...
#define CAN_READ_XOID(bChannel) \
    (   ((CAN_OBJ[bChannel].id_high.id_high & 0xffe0) << 13) \
...
#define CAN_WRITE_OID(bChannel, Id) \
	    (CAN_OBJ[bChannel].id_high.std_id = (Id) << 5)
