...
struct _NodeAnalogExtObjectPrivate {
    gboolean temp;
};

G_DEFINE_TYPE_WITH_PRIVATE(NodeAnalogExtObject, node_analog_ext_object, NODE_TYPE_OBJECT);
...
static void node_analog_ext_object_init(NodeAnalogExtObject *analog) {
    NodeAnalogExtObjectPrivate *priv = node_analog_ext_object_get_instance_private(analog);
    analog->priv                     = priv;
}

static void node_analog_ext_object_constructed(GObject *object) {
...
static void node_analog_ext_object_finalize(GObject *object) {
    /* TODO: Add deinitalization code here */
    // NodeAnalogExtObject *analog = NODE_ANALOGEXT_OBJECT(object);
    G_OBJECT_CLASS(node_analog_ext_object_parent_class)->finalize(object);
}

static void node_analog_ext_object_set_property(GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec) {
    g_return_if_fail(NODE_IS_ANALOGEXT_OBJECT(object));
    // NodeAnalogExtObject *analog = NODE_ANALOGEXT_OBJECT(object);
    switch (prop_id) {
...
static void node_analog_ext_object_get_property(GObject *object, guint prop_id, GValue *value, GParamSpec *pspec) {
    g_return_if_fail(NODE_IS_ANALOGEXT_OBJECT(object));
    // NodeAnalogExtObject *analog = NODE_ANALOGEXT_OBJECT(object);
    switch (prop_id) {
    default:
...
static void node_analog_ext_object_class_init(NodeAnalogExtObjectClass *klass) {
    GObjectClass *object_class = G_OBJECT_CLASS(klass);
    // NodeObjectClass* parent_class = NODE_OBJECT_CLASS (klass);

    object_class->finalize     = node_analog_ext_object_finalize;
    object_class->set_property = node_analog_ext_object_set_property;
    object_class->get_property = node_analog_ext_object_get_property;
    object_class->constructed  = node_analog_ext_object_constructed;
}
