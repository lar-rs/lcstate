				     ...
				     gpointer user_data)
{
	...
	{
		mkt_task_status(MKT_TASK(rinsing_task),"Injection go rinsing failed - %s",error?error->message:"unknown");
		if(error)g_error_free(error);
		rinsing_task_failed(rinsing_task);
	}
	else
	{
		...
		{
			mkt_task_status(MKT_TASK(rinsing_task),"Close injection valve fail - %s",error?error->message:"unknown");
			if(error)g_error_free(error);
			rinsing_task_failed(rinsing_task);
			return;
		}
		mkt_task_status(MKT_TASK(rinsing_task),"Injection go to hold");
		...
		gpointer user_data)
{
	...
	{
		mkt_task_status(MKT_TASK(rinsing_task),"Z - go to hold position failed - %s",error?error->message:"unknown");
		if(error)g_error_free(error);
		rinsing_task_failed(rinsing_task);
	}
	else
	{
		if(rinsing_task->priv->repeat <  rinsing_task->priv->replicate)
		{
			...
			if(!valves_simple_call_open_sync(valves_object_get_simple(ULTRA_VALVE_INJECTION()),&is_done,NULL,&error))
			{
				...
				mkt_task_status(MKT_TASK(rinsing_task),"Open injection valve fail - %s",error?error->message:"unknown");
				if(error)g_error_free(error);
				rinsing_task_failed(rinsing_task);
				return;
			}
			rinsing_task->priv->repeat++;
			mkt_task_status(MKT_TASK(rinsing_task),"Injection go to rinsing");
		...
		else
		{
			...
			mkt_task_status(MKT_TASK(rinsing_task),"Y - go back to hold position");
		...
		gpointer user_data)
{
	...
	else
	{
		...
		mkt_task_status(MKT_TASK(rinsing_task),"Injection go to sensor");
				     ...
				     gpointer user_data)
{
	...
	{
		mkt_task_status(MKT_TASK(rinsing_task),"Y - go to needle position failed - %s",error?error->message:"unknown");
		if(error)g_error_free(error);
		rinsing_task_failed(rinsing_task);
	}
	else
