...
static void node_device_object_create_scan_node (NodeDeviceObject* device)
{
	gchar* etc_path = g_strdup_printf ("%s/candaemon/eds/Scan.eds", SYSCONFDIR);

	device -> priv -> scan = NODE_OBJECT (g_object_new (NODE_TYPE_OBJECT, "g-object-path", "/com/lar/nodes/Scan", "node-configuration", etc_path, NULL));

	nodes_simple_set_device             (nodes_object_get_simple(NODES_OBJECT(device->priv->scan)),      g_dbus_object_get_object_path(G_DBUS_OBJECT(device)));
	nodes_simple_set_node_id            (nodes_object_get_simple(NODES_OBJECT(device->priv->scan)),      1);
	g_dbus_object_manager_server_export (G_DBUS_OBJECT_MANAGER_SERVER(node_control_app_nodes_manager()), G_DBUS_OBJECT_SKELETON(device->priv->scan));
	g_free                              (etc_path);
}
...
gboolean node_device_object_write_index (NodeDeviceObject* device, NodeIndex* index)
{
	...
	switch (node_index_datatype (index)) {
		case 0x1:
		...
		case 0x9: cm . data [0] = 0x21; break;  // n Byte
		default:  cm . data [0] = 0x23; break;
	}

	value = node_index_value32(index);
